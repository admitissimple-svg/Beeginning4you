...
export default function BeeLogo({
  ...
  const renderBeeArtwork = (strokeColor: string, strokeWidth = 3.2) => (
    <g transform="translate(216, 28) rotate(12)">
      ...
      <div className={`relative inline-flex flex-col items-center select-none ${className}`}>
        ...
        <div className="relative flex items-end justify-center tracking-tight pt-8 sm:pt-10">
          ...
          <svg
            ...
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className={`${scale.beeH} w-auto overflow-visible shrink-0 select-none pb-0.5 -mr-3 sm:-mr-4`}
            aria-label="Bee"
          >
            {/* Baguet Script glyph path for "Bee" */}
            <path d={BEE_VECTOR_PATH} fill={lineColor} />

            {/* Seamless continuous, gently sinuous flight wave exiting the second 'e' terminal at (144.4, 76.5) and flowing directly into the infinity bee at (216, 28) */}
            <path
              d="M 144.4 76.5 C 151 59, 164 54, 177 62 C 190 70, 204 50, 216 28"
              stroke={lineColor}
              strokeWidth="3.2"
              strokeLinecap="round"
              strokeLinejoin="round"
            ...
            {renderBeeArtwork(lineColor, 3.2)}
          </svg>

          {/* "ginning" in ADLaM Display */}
          <span
            className={`font-adlam ${scale.ginning} ${primaryTextColor} font-normal lowercase tracking-tight leading-none pl-1`}
          >
            ginning
          </span>
        </div>
